...
const _canHash = typeof crypto !== 'undefined' && crypto.subtle && crypto.subtle.digest;

function _sha256_js(buffer) {
    ...
    const lenBits = u8.length * 8;
    const pad = new Uint8Array((u8.length + 9 + 63) & ~63);
    pad.set(u8); pad[u8.length] = 0x80;
    const dv = new DataView(pad.buffer);
    dv.setUint32(pad.length - 8, Math.floor(lenBits / 0x100000000), false);
    dv.setUint32(pad.length - 4, lenBits >>> 0, false);
        ...
        conn.on('data', data => {
            ...
            if (data.type === 'file-ack' || data.type === 'file-bulk-ack') {
                ...
                for (const idx of indices) {
                    tx.peerAcks[sid].add(idx);
                    tx._inFlight.delete(idx);
                    if (tx._timers.has(idx)) { clearTimeout(tx._timers.get(idx)); tx._timers.delete(idx); }
                }
